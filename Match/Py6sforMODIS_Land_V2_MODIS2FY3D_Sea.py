...
KEY = 'MERSI'
Month = 'March'
sOutFile = r'C:\Users\1\Desktop\casestudy\data\MatchRes/'+Month+'/Py6s_'+KEY+'_Sea_test.csv'
Timetable = [2,5,8,10,13,16,18,21,24,29]
MatchTablePath = r"D:\MatchRes\March\LandMatchTable_MERSI_MODIS_03_Sea.csv"
# MatchTablePath = r"D:\MatchRes\March\LandMatchTable_MERSI_AQUA_Newlib3.csv"
...
AeroMODISpath = r'C:\Users\1\Desktop\casestudy\data\ERA5/Aero/'+Month+'/MODIS_Aero'
HeightPath = r"D:\landfile\GlobalDEM-GOTOP30_0.25X0.25.dat"
Bias = 0
...
for i in np.arange(len(MatchTab)):
    ...
    if MatchTab.loc[i, 'Land/Sea'] == 'S':
        ...
        #         break
        # wk  = i
        # AeroMODIS = get_aero_MODIS(AeroMODISfile,MatchTab.loc[wk, 'iPos'], MatchTab.loc[wk, 'jPos']) 
        # Aero  = AeroMODIS
        ...
        Chlafiles = glob.glob(Chlapath +'/*.dat')
        Chlafile = []
        Y_M_D = ERA5time.strftime('%Y-%m-%d') 
        KeywordChla = '{}{}{}'.format(str(ERA5time.year), str(ERA5time.month).zfill(2),str(ERA5time.day).zfill(2))

        for k in Chlafiles:
            # 确定日期
            if KeywordChla in os.path.basename(k):
                # print('Find Match ' + time4Y2M2D)  # 绝对路径
                Chlafile  = k
                break
        Chlafile_Sample = np.fromfile(Chlafile, dtype=np.float32)
        Chlafile_Sample.shape = 721, 1440
        Chla = np.array([Chlafile_Sample[MatchTab.loc[i, 'iPos'], MatchTab.loc[i, 'jPos']]])
        ...
        # aerosol
        X.aero_profile = AeroProfile.PredefinedType(AeroProfile.Maritime)
        X.aot550 = Aero[0]
        ...
        for j in np.arange(nBand) :
            ...
            Day = ERA5time.day
            Hour = ERA5time.hour
            p = MatchTab.loc[i,'iPos']
            q = MatchTab.loc[i,'jPos']
            Dayindex = Timetable.index(Day)
            Hour = ERA5time.hour
            index = Timetable.index(Day)
            uWind = u.variables['u'][(index)*24+Hour][0,p,q]
            vWind = v.variables['v'][(index)*24+Hour][0,p,q]
            WindSpeed = math.sqrt(uWind**2+vWind**2)
            wdir1 =   180+ np.arctan2(uWind, vWind)*(180.0/np.pi)


            X.ground_reflectance = GroundReflectance.HomogeneousOcean(WindSpeed,wdir1,34.3,Chla)
