...
KEY = 'VIIRS'
Month = 'March'
sOutFile = r'C:/Users/1/Desktop/casestudy/data/MatchRes/'+Month+'/Py6s_'+KEY+'_'+Month+'.csv'
dTime = 900
MatchTablePath = r'C:\Users\1\Desktop\casestudy\data\MatchRes/'+Month+'/LandMatchTable_MERSI_VIIRS_'+Month+'.csv'
# MatchTablePath = r"C:/Users/1/Desktop/casestudy/data/MatchRes\March\LandMatchTable_MERSI_AQUA_Newlib3.csv"
BRDFpath = r'C:/Users/1/Desktop/casestudy/data/MODIS/'+Month+'/BRDF/BRDFC1_Sampleto_11CH_'+KEY
...
VIIRS = [PredefinedWavelengths.VIIRS_BM1,
...
def get_aero(aeropath,iPos,jPos):
        ...
        return AOD
def get_aero_VIIRS(aeropath,iPos,jPos):
            ...
            nc = Dataset(aeropath,'r')
            # AeroVIIRS 的lon从-180，180，lat从-90，90，大小为180*360
            ib = 90+ib
...
fp = open(sOutFile, 'w')
fp.write('lat,lon,ipos,jpos,B1o,B1s,Diff,B2o,B2s,Diff,B3o,B3s,Diff,B4o,B4s,Diff,B5o,B5s,Diff,B6o,B6s,Diff,B7o,B7s,Diff,B8o,B8s,Diff,B9o,B9s,Diff,B10o,B10s,Diff,B11o,B11s,Diff,NDVI,SZA,SAA,VZA,VAA,CLM,Time,Num,Height,AOD550,dTime\n')
nBand = 11
...
for i in np.arange(len(MatchTab)):
    ...
    # if MatchTab.loc[i, 'NDVI'] <= 0.3:  continue
    # if MatchTab.loc[i, 'NDVI'] >= 0.6:  continue

    if MatchTab.loc[i,'Land/Sea'] == 'S' :continue
        ...
        # AeroVIIRSfiles = glob.glob(AeroVIIRSpath +'/*.nc')
        # AeroVIIRSfile = []
        # days = (ERA5time - time0).days + 1
        # AKeywordVIIRS = 'A{}{}'.format(str(ERA5time.year), str(days).zfill(3))

        # for k in AeroVIIRSfiles:
        #     # 确定日期
        #     if AKeywordVIIRS in os.path.basename(k):
        #         # print('Find Match ' + time4Y2M2D)  # 绝对路径
        #         AeroVIIRSfile = k
        #         break
        # k  = i
        # AeroVIIRS = get_aero_VIIRS(AeroVIIRSfile,MatchTab.loc[k, 'iPos'], MatchTab.loc[k, 'jPos']) 
        # Aero  = AeroVIIRS
        ...
        for j in np.arange(nBand) :
            ...
            # 波长
            # Band = 'PredefinedWavelengths.ACCURATE_VIIRS_AQUA_'+'{}'.format(j+1)
            ...
            BRDFP2 = BRDF[BRDFBand + 11, MatchTab.loc[i, 'iPos'], MatchTab.loc[i, 'jPos']]
            BRDFP3 = BRDF[BRDFBand + 22, MatchTab.loc[i, 'iPos'], MatchTab.loc[i, 'jPos']]
            ...
            # X.ground_reflectance = GroundReflectance.HomogeneousVIIRSBRDF(0.1,0.1,0.1)
            X.run()
